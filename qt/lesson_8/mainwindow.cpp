...
MainWindow::MainWindow(QWidget *parent)
    ...
    connect(ui->mPasteDate, &QAction::triggered, this,
            &MainWindow::on_mPasteDate_trigger);
    connect(ui->pbSearchEdit, &QPushButton::clicked, this,
            &MainWindow::pushSearch);
...
void MainWindow::pushSearch() {
    QFileInfo cp(ui->lineEditCurPath->text());
    QString cPath = "";
    if (cp.isFile()) {
        cPath = cp.absolutePath();
    } else {
        cPath = cp.filePath();
    }
    QString se = ui->searchEdit->text();
    if (cPath == "" || se == "") {
        return;
    }

    ui->pbSearchEdit->setEnabled(false);

    Finder *finder = new Finder;
    finder->moveToThread(&searchThread);

    connect(&searchThread, &QThread::finished, finder, &QObject::deleteLater);
    connect(this, &MainWindow::startSearch, finder, &Finder::doFind);
    connect(finder, &Finder::resultReady, this, &MainWindow::searchResult);
    searchThread.start();

    emit startSearch(cPath, se);
}

void MainWindow::searchResult(QVector<QString> results) {
    searchThread.quit();
    searchThread.wait();

    QWidget *resultSearchW = new QWidget();
    resultSearchW->setWindowFlags(Qt::WindowCloseButtonHint);
    resultSearchW->setWindowModality(Qt::ApplicationModal);
    resultSearchW->setAttribute(Qt::WA_DeleteOnClose);
    resultSearchW->setWindowTitle("Результаты поиска:");
    int w = 450;
    int h = 500;
    resultSearchW->setGeometry(this->geometry().center().x() - w / 2,
                               this->geometry().center().y() - h / 2, w, h);
    QBoxLayout *bl = new QBoxLayout(QBoxLayout::TopToBottom, resultSearchW);
    QTextEdit *resultTextEdit = new QTextEdit(resultSearchW);
    resultTextEdit->setReadOnly(true);
    bl->addWidget(resultTextEdit);

    for (auto i : results) {
        resultTextEdit->textCursor().insertText(i + "\n");
    }
    resultSearchW->show();
    ui->pbSearchEdit->setEnabled(true);
}

void MainWindow::on_mPasteDate_trigger() {
    if (!ui->mdiArea->activeSubWindow()) {
        return;
    }
    QDateTime dt;
    auto te =
        qobject_cast<QTextEdit *>(ui->mdiArea->activeSubWindow()->widget());
    te->textCursor().insertText(
        dt.currentDateTime().toString(" dd-MM-yyyy hh:mm "));
}
...
void MainWindow::shortCutSetup() {
    ...
    ui->mPasteDate->setShortcut(QKeySequence(mydata.at(9)));
}
